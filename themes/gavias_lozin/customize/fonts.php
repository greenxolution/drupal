<?php
function gavias_lozin_font_size(){
    $size = array();
    $size[''] = '-- Default --';
    for ($i=12; $i <= 50 ; $i = $i+1) { 
        $size[$i] = $i;
...
function gavias_lozin_render_option_font(){
    $fonts = gavias_lozin_fonts();
    $output = '';
...
function gavias_lozin_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function gavias_lozin_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(gavias_lozin_fonts())) > 17){
...
function gavias_lozin_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode((string)$json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= gavias_lozin_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= gavias_lozin_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function gavias_lozin_options_patterns(){
    $output = '';
    $file_path = \Drupal::service('extension.list.theme')->getPath('gavias_lozin');
