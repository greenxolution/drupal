  ...
  public function testNewsletterIssuesOverview() {
    ...
    foreach ($rows as $row) {
      if ($row->find('xpath', '/td[2]/a')->getText() == 'Test_issue_2') {
        ...
        $this->assertEqual(\Drupal::service('file_url_generator')->transformRelative(file_create_url(\Drupal::service('extension.list.module')->getPath('simplenews') . '/images/sn-cron.png')), trim($row->find('xpath', '/td[6]/img')->getAttribute('src')));
      }
    }
    ...
    foreach ($rows as $row) {
      if ($row->find('xpath', '/td[2]/a')->getText() == 'Test_issue_2') {
        ...
        $this->assertEqual(\Drupal::service('file_url_generator')->transformRelative(file_create_url(\Drupal::service('extension.list.module')->getPath('simplenews') . '/images/sn-sent.png')), trim($row->find('xpath', '/td[6]/img')->getAttribute('src')));
      }
    }
  }
